        ...
        <form action="{{ route('sale.data.create') }}" method="POST" id="your-form" enctype="multipart/form-data"
                        ...
                        <div class="card-body">
                            ...
                            <input type="text" class="form-control" id="sale_on" name="sale_on" required >
                                ...
                                <div class="mb-3 col-md-4" id="final_price">
                                    <label for="plan_price" class="form-label">方案追加/尾款價格<span
                                            class="text-danger">*</span></label>
            ...
            <div class="row not_memorial_show" id="product_div">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="text-uppercase bg-light p-2 mt-0 mb-3">商品選購</h5>
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="table-responsive">
                                        <table id="tech-companies-1" class="table product-list">
                                            <thead>
                                                <tr>
                                                    <th></th>
                                                    <th>商品類別<span class="text-danger">*</span></th>
                                                    <th>商品名稱<span class="text-danger">*</span></th>
                                                    <th>數量<span class="text-danger">*</span></th>
                                                    <th>售價<span class="text-danger">*</span></th>
                                            ...
                                            <tbody>
                                                ...
                                                @for ($i = 0; $i < 1; $i++)
                                                    ...
                                                    <tr id="row-{{ $i }}">
                                                        ...
                                                        <td>
                                                            <select id="product_categories_{{ $i }}"
                                                                alt="{{ $i }}" class="mobile form-select"
                                                                name="product_categories[]"
                                                                onchange="chgCategories(this)">
                                                                <option value="" selected>請選擇...</option>
                                                        ...
                                                        <td>
                                                            <select id="product_id_{{ $i }}"
                                                                alt="{{ $i }}" class="mobile form-select"
                                                                name="product_ids[]" onchange="chgProducts(this)">
                                                                <option value="" selected>請選擇...</option>
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <input type="number" alt="{{ $i }}"
                                                                class="mobile form-control"
                                                                id="product_num_{{ $i }}" name="product_num[]"
                                                                onchange="chgNums(this)" onclick="chgNums(this)"
                                                                onkeydown="chgNums(this)">
                                                        </td>
                                                        <td>
                                                            <input type="text" class="mobile form-control total_number"
                                                                id="product_total_{{ $i }}"
                                                                name="product_total[]" value="">
                                                        </td>
                                                        <td>
                                                            <input type="text" class="mobile form-control"
                                                                id="product_comment_{{ $i }}"
                                                                name="product_comment[]" value="">
                                                        </td>
                                                    </tr>
                                                @endfor
                                            </tbody>
    ...
    <script>
        ...
        $("table.gdpaper-list tbody").on("click", ".ibtnAdd_gdpaper", function() {
            ...
            cols += '<input type="number"  alt="' + rowCount + '"  class="mobile form-control" id="gdpaper_num_' +
                rowCount +
                '" name="gdpaper_num[]" value="" onchange="chgNums(this)" onmousedown="chgNums(this)" onkeydown="chgNums(this)">';
            cols += '</td>';
            cols += '<td>';
        ...
        // <----紀念品--->
        $("table.product-list tbody").on("click", ".ibtnDel_product", function() {
            $(this).closest('tr').remove();
        });

        $("table.product-list tbody").on("click", ".ibtnAdd_product", function() {
            rowCount = $('table.product-list tr').length - 1;
            var newRow = $("<tr>");
            var cols = '';
            cols +=
                '<td class="text-center"><button type="button" class="ibtnDel_product demo-delete-row btn btn-danger btn-sm btn-icon"><i class="fa fa-times"></i></button></td>';
            cols += '<td>';
            cols += '<select id="product_categories_' + rowCount + '"';
            cols += ' alt="' + rowCount + '" class="mobile form-select"';
            cols += 'name="product_categories[]"';
            cols += 'onchange="chgCategories(this)">';
            cols += '<option value="" selected>請選擇...</option>';
            @foreach ($product_categories as $product_category)
                cols += '<option value="{{ $product_category->id }}">';
                cols += ' {{ $product_category->name }}';
                cols += '</option>';
            @endforeach
            cols += '</select>';
            cols += '</td>';
            cols += '<td>';
            cols += ' <select id="product_id_' + rowCount + '"';
            cols += ' alt="' + rowCount + '" class="mobile form-select"';
            cols += ' name="product_ids[]" onchange="chgProducts(this)">';
            cols += ' <option value="" selected>請選擇...</option>';
            cols += ' </select>';
            cols += '</td>';
            cols += '<td>';
            cols += ' <input type="number" alt="' + rowCount + '"';
            cols += '  class="mobile form-control"';
            cols += '   id="product_num_' + rowCount + '" name="product_num[]"';
            cols += '   onchange="chgNums(this)" onclick="chgNums(this)"';
            cols += '    onkeydown="chgNums(this)">';
            cols += '</td>';
            cols += '<td>';
            cols += '<input type="text" class="mobile form-control total_number"';
            cols += ' id="product_total_' + rowCount + '"';
            cols += ' name="product_total[]" value="">';
            cols += '</td>';
            cols += '<td>';
            cols += '<input type="text" class="mobile form-control"';
            cols += ' id="product_comment_' + rowCount + '"';
            cols += ' name="product_comment[]" value="">';
            cols += '</td>';
            cols += '</tr>';
            newRow.append(cols);
        ...
        function chgCategories(obj) {
            $("#row_id").val($("#" + obj.id).attr('alt'));
            row_id = $("#row_id").val();
            console.log(row_id);
            $.ajax({
                url: '{{ route('ajax.product_search') }}',
                data: {
                    'category_id': $("#product_categories_" + row_id).val()
                },
                success: function(data) {
                    $("#product_id_" + row_id).html(data);
                }
            });
        }

        function chgProducts(obj) {
            $("#row_id").val($("#" + obj.id).attr('alt'));
            row_id = $("#row_id").val();
            console.log(row_id);
            $.ajax({
                url: '{{ route('ajax.product_search_price') }}',
                data: {
                    'product_id': $("#product_id_" + row_id).val()
                },
                success: function(data) {
                    if ($("#product_num_" + row_id).val()) {
                        var product_num = $("#product_num_" + row_id).val();
                        $("#product_total_" + row_id).val(product_num * data);
                        calculate_price();
                    } else {
                        $("#product_num_" + row_id).on('change', function() {
                            var product_num = $("#product_num_" + row_id).val();
                            $("#product_total_" + row_id).val(product_num * data);
                            calculate_price();
                        });
