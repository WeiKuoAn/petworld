...
<div class="container-fluid">
            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">編輯法會</li>
                    </ol>
                </div>
                <h5 class="page-title">編輯法會</h5>
            </div>
        </div>
    </div>
    <!-- end page title -->

    <form action="{{ route('puja.edit.data',$data->id) }}" method="POST" id="your-form"  enctype="multipart/form-data" data-plugin="dropzone" data-previews-container="#file-previews" data-upload-preview-template="#uploadPreviewTemplate">
        @csrf
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="mb-3 col-md-4">
                            <label for="type" class="form-label">法會類別<span class="text-danger">*</span></label>
    ...
    <div class="row" id="gdpaper_div">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <h5 class="text-uppercase bg-light p-2 mt-0 mb-3">法會附加品項</h5>
                    <div class="row">
                        <div class="col-md-12">
                            <div class="table-responsive">
                                <table id="tech-companies-1" class="table gdpaper-list">
                                    ...
                                    <tbody>
                                        @foreach($puja_products as $key=>$puja_product)
                                            <tr id="row-{{ $key }}">
                                            ...
                                            <td>
                                                <select id="gdpaper_id_{{$key }}" alt="{{ $key  }}" class="mobile form-select" name="gdpaper_ids[]" onchange="chgPapers(this)" >
                                                    ...
                                                    @foreach($products as $product)
                                                        <option value="{{ $product->id }}" @if($puja_product->product_id == $product->id) selected @endif>{{ $product->name }}({{ $product->price }})</option>
                                                    @endforeach
                                                </select>
                                            </td>
                                            <td>
                                                <input type="number" class="mobile form-control" alt="{{ $key  }}"   id="gdpaper_num_{{$key }}" name="gdpaper_num[]" value="{{ $puja_product->product_num }}" onchange="chgNums(this)">
                                            </td>
                                            <td>
                                                <input type="text" class="mobile form-control total_number" id="gdpaper_total_{{$key }}" name="gdpaper_total[]" value="{{ $puja_product->product_total }}">
                                            </td>
                                        </tr>
                                        @endforeach
                                    </tbody>
...
<script>
    ...
    $('select[name="gdpaper_ids"]').on('change', function(){
        console.log(1);
    });
    ...
    $("table.gdpaper-list tbody").on("click", ".ibtnAdd_gdpaper", function() {
        ...
        cols += '</td>';
        cols += '<td>';
        cols += '<input type="number" class="mobile form-control" id="gdpaper_num_'+rowCount+'" name="gdpaper_num[]" value="" onchange="chgNums(this)">';
        cols += '</td>';
        cols += '<td>';
