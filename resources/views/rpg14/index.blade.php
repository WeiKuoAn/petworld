            ...
            <div class="card">
                <div class="card-body">
                    <div class="row justify-content-between">
                        <div class="col-auto">
                            <form class="d-flex flex-wrap align-items-center" action="{{ route('rpg14') }}" method="GET">
                            ...
                            <thead class="table-light">
                                ...
                                <tbody>
                                    <tr align="center" style="color:red;font-weight:500;">
                                        <td>總單量</td>
                                        @foreach ($sources as $key => $source)
                                            @if (isset($sums[$source->code]['count']) && $sums[$source->code]['count'] != 0)
                                                <td>{{ $sums[$source->code]['count'] }}單</td>
                                            @else
                                                <td>0單</td>
                                            @endif
                                        @endforeach
                                    ...
                                    @foreach ($datas as $key => $data)
                                        <tr align="center">
                                            ...
                                            @foreach ($sources as  $source)
                                                @if (isset($data[$source->code]['count']) && $data[$source->code]['count'] != 0)
                                                    <td><a href="{{ route('rpg14.detail',['date'=>$key,'source_code'=>$source->code]) }}">{{ $data[$source->code]['count'] }}</a></td>
                                                @else
                                                    <td></td>
                                                @endif
                                            @endforeach
