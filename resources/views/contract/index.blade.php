                                    ...
                                    <div class="me-sm-3 mt-1">
                                        ...
                                        <select class="form-select my-1 my-lg-0" id="status-select" name="status"
                                            ...
                                            @if (!isset($request->type) || $request->type=="null")
                                                ...
                                                <option value="8" @if ($request->status == '8') selected @endif>
                                                    已使用未key單</option>
                                                <option value="9" @if ($request->status == '9') selected @endif>
                                                    已使用並key單</option>
                                                <option value="10" @if ($request->status == '10') selected @endif>
                                                    已結案</option>
                                            @elseif ($request->type == '1')
                                                {{-- 生前契約專用狀態 --}}
                                                <option value="5" @if ($request->status == '5') selected @endif>
                                                    已退款</option>
                                                <option value="8" @if ($request->status == '8') selected @endif>
                                                    已使用未key單</option>
                                                <option value="9" @if ($request->status == '9') selected @endif>
                                    ...
                                    <a href="{{ route('contract.create') }}">
                                        <button type="button" class="btn btn-danger waves-effect waves-light"
                                            data-bs-toggle="modal" data-bs-target="#custom-modal"><i
                                                class="mdi mdi-plus-circle me-1"></i>新增契約</button>
                                    </a>
                                </div>
                            </div><!-- end col-->
                        </div> <!-- end row -->
                            ...
                            <table class="table table-centered table-nowrap table-hover mb-0">
                                <thead class="table-light">
                                    <tr>
                                        ...
                                        @endif
                                        <th>金額</th>
                                        @if ($request->status != '9' && $request->status != '10')
                                            <th>動作</th>
                                        @endif
                                    </tr>
                                </thead>
