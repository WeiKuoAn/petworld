...
class InventoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($request->category_id == 'all'){
          $products = Product::orderby('seq','asc')->orderby('price','desc')->where('stock','1')->where('type', '!=', 'combo')->where('status','up')->get();
        }else{
          $products = Product::orderby('seq','asc')->orderby('price','desc')->where('category_id',$request->category_id)->where('type', '!=', 'combo')->where('stock','1')->where('status','up')->get();
        }
        // dd($products);
